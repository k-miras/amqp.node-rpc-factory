...
var rpcConsumerProto = {
  ...
  init: function(options) {
    ...
    this.handleOnce = options.handleOnce || false;
  ...
  connect: function() {
    amqp
      .connect(this.uri(), this.socketOptions)
      .then(
        function getConnectionSuccess(conn) {
          conn.on(
            "error",
            function(err) {
          ...
          return conn.createChannel().then(
            function createChannelSuccess(ch) {
              // Capitalize this function as its used with a this binding
              function Reply(msg) {
                // process request
                var response;

                if (this.handleOnce == true) {
                  ch.checkQueue(msg.properties.replyTo).then( ok => {
              ...
              return ch
                .assertQueue(this.queue, this.queueOptions)
                .then(
                ...
                .then(
                  function consumeSuccess() {
                    this.logInfo(
                      "Consumer: Waiting for RPC requests on: " + this.queue
                    );
                  }.bind(this)
                );
            }.bind(this)
